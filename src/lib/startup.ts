...
import type { Category, NewNotification, Notification, NewTransaction, RecurringExpense } from "src/types";
...
import { deleteNotification, getNotifications, saveNotification } from "./services/notificationService";
...
export async function handleStartup(categories: Category[]): Promise<void> {
  ...
  if (lastOpened.getDate() == today.getDate() && 
...
async function processRecurringExpenses(categories: Category[], lastOpened: Date): Promise<void> {
  ...
  for (const expense of recurringExpenses) {
    ...
    else if (expense.recurs === 'yearly' && expense.month === today.getMonth() + 1 && expense.day === today.getDate()) {
      await applyRecurringExpense(expense, categories);
      await applyExpenseNotification(expense, 0);
    }
    // Expense has been due since last time application was opened
    else if (expense.recurs === 'monthly' && today.getDate() > expense.day && lastOpened.getDate() < expense.day) {
      await applyRecurringExpense(expense, categories);
      await applyExpenseNotification(expense, expense.day - today.getDate());
    }
    else if (expense.recurs === 'yearly' && expense.month === today.getMonth() + 1 && today.getDate() > expense.day && lastOpened.getDate() < expense.day) {
      await applyRecurringExpense(expense, categories);
      await applyExpenseNotification(expense, expense.day - today.getDate());
    }
    // Expense notification date has passed since last time application was opened
    else if (expense.recurs === 'monthly' && today.getDate() >= notificationDay && notificationDay > lastOpened.getDate()) {
      await applyExpenseNotification(expense, expense.day - today.getDate());
    }
    else if (expense.recurs === 'yearly' && expense.month === today.getMonth() + 1 && today.getDate() >= notificationDay && notificationDay > lastOpened.getDate()) {
      await applyExpenseNotification(expense, expense.day - today.getDate());
    }
  }
}

async function applyRecurringExpense(expense: RecurringExpense, categories: Category[]): Promise<void> {
  const today = new Date();
